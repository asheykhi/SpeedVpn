...
public class VpnProfile implements Serializable, Cloneable {
    ...
    private boolean mUseDefaultRoutev6 = true;
    ...
    public static String openVpnEscape(String unescaped) {
        ...
        if (escapedString.equals(unescaped) && !escapedString.contains(" ") &&
                !escapedString.contains("#") && !escapedString.contains(";") && !escapedString.equals("")) return unescaped;
    ...
    public String getName() {
        if (mName == null) return "No profile name";
        return mName;
    }
    ...
    private String getConfigFile(Context context, boolean configForOvpn3) {
        ...
        if (mOverrideDNS || !mUsePull) {
            if (!TextUtils.isEmpty(mDNS1)) cfg += "dhcp-option DNS " + mDNS1 + "\n";
...
//        }
    ...
    public int needUserPWInput(boolean ignoreTransient) {
        if ((mAuthenticationType == TYPE_PKCS12 || mAuthenticationType == TYPE_USERPASS_PKCS12) && (mPKCS12Password == null || mPKCS12Password.equals(""))) {
            if (ignoreTransient || mTransientPCKS12PW == null) return R.string.pkcs12_file_encryption_key;
        }
        if (mAuthenticationType == TYPE_CERTIFICATES || mAuthenticationType == TYPE_USERPASS_CERTIFICATES) {
            if (requireTLSKeyPassword() && TextUtils.isEmpty(mKeyPassword)) if (ignoreTransient || mTransientPCKS12PW == null) {
                return R.string.private_key_password;
            }
        }
        if (isUserPWAuth() && (TextUtils.isEmpty(mUsername) || (TextUtils.isEmpty(mPassword) && (mTransientPW == null || ignoreTransient)))) {
    ...
    private class NoCertReturnedException extends Exception {
